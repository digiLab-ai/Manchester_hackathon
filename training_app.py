...
diffusivity_min, diffusivity_max = st.sidebar.slider(
    f"{X3_name} input range [{X3_units}]", 0.1, 10.0, (0.1, 10.0)
)
solubility_min, solubility_max = st.sidebar.slider(
    f"{X2_name} input range [{X2_units}]", 0.01, 0.1, (0.01, 0.1)
)
thickness_min, thickness_max = st.sidebar.slider(
    f"{X1_name} input range [{X1_units}]", 1.0, 3.0, (1.9, 2.0), step=0.1
)
...
X, Y, Z = np.meshgrid(
    np.linspace(bounds[X1_name][0], bounds[X1_name][1], num=npoints),
...
uncertainty_cutoff = (
    st.sidebar.slider(
        "Model quality cutoff (Average Uncertainty %)", 0.0, 100.0, 10.0, step=0.01
    )
    / 100
)
...
if st.sidebar.button("Train Model"):
    ...
    df_input = {"diffusivity": [], "solubility": [], "thickness": [], "output_flow": []}
    try:
    ...
    df_score = pd.DataFrame({"batches": batches, "UQ_score_history": UQ_score_history})
    ...
    if not emulator_performs:
        ...
        while uncertainty_score > uncertainty_cutoff:
            ...
            df_input = engine.get_data(
                client=client, project_name=PROJECT_NAME, dataset_name=dataset_name
            )
            ...
            engine.train_and_save_model_workflow(
                client,
                project_name=PROJECT_NAME,
            ...
            df_score = pd.DataFrame(
                {"batches": batches, "UQ_score_history": UQ_score_history}
            )
            ...
            progress.progress(
                min(int(uncertainty_cutoff / uncertainty_score * 100), 100)
            )
        ...
        status.text(
            f"uncertainty is {100*round(uncertainty_score,3)} %, training complete!"
        )
